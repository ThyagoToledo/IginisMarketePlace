  ...
  try {
    ...
    if (users[0]) {
      const items = await sql`
        ...
        FROM items WHERE author_id = ${users[0].id} AND status = 'approved'
    ...
    const legacyItems = await sql`
      SELECT id, type, name, author, description, version,
             git_url AS "gitUrl", cover_image_text AS "coverImageText",
             dependencies, downloads
      FROM items WHERE lower(author) = lower(${username}) AND status = 'approved'
      ORDER BY downloads DESC, created_at DESC`;
...
export async function loadAdminStats() {
  try {
    const sql = getSql();
    const [users, items, downloads, promotions] = await Promise.all([
    ...
    return {
      ...
      ignisFeatured: 0, sponsoredFeatured: 0, offline: true,
    };
  }
}
