...
import { loadProfileActivity } from './community-data';
...
export async function loadCreator(username) {
  try {
    ...
    if (users[0]) {
      const [items, activityRows] = await Promise.all([
        sql`SELECT id, type, name, author, description, version,
        ...
        loadProfileActivity(users[0].id),
      ]);
      return { ...users[0], items, activityRows };
    }
    ...
    return { username, displayName: username, avatarUrl: null, items: legacyItems, activityRows: [] };
  } catch (error) {
